...
Dari **79.561 alert** yang dianalisis:
- **79.378 alert (99.8%)** adalah **False Positive** — terutama traffic internal dari IP `10.0.0.5` (Azure subnet)
- **183 alert (0.2%)** adalah **True Positive** — serangan SYN flood nyata dari IP eksternal + malware detection

Ini membuktikan bahwa tanpa AI filtering, SOC analyst harus mereview **hampir 80 ribu alert** untuk menemukan **hanya 183 ancaman nyata** — rasio yang sangat tidak efisien dan menyebabkan alert fatigue.
...
|---|---|---|---|---|
| 100200 | SYN Flood (iptables) | 37.359 | 180 | 37.179 |
...
|---|---|---|
| 1–12 | 136 | Low-frequency — campuran TP (external scan) dan FP |
| 46 | 46 | Moderate — SSH brute-force dari 1 IP |
| 60 | 60 | High — SSH brute-force sustained dari 1 IP |
| 483 | 483 | Tinggi — Azure health probe (FP) |
| 1.101–1.311 | 2.412 | Sangat tinggi — DNS resolver loopback (FP) |
| 76.424 | 76.424 | Masif — internal Azure traffic (FP) |
...
- **Raw:** `ai-model/data/raw_alerts.csv` — 79.561 baris, semua alert
- **Labeled:** `ai-model/data/labeled_alerts.csv` — 250 baris tersampling, berlabel

### 5.2 Format CSV Final

```csv
rule_id,rule_level,freq_per_minute,hour_of_day,src_port,dst_port,label
100402,12,45,2,51234,80,1
100400,1,3,14,33421,80,0
```
...
|---|---|---|
| True Positive (label=1) | 162 | 64.8% |
| False Positive (label=0) | 88 | 35.2% |
...
1. **99.8% alert adalah false positive** — tanpa AI, analyst harus mereview ~80K alert untuk menemukan 183 ancaman nyata
2. **UDP Flood 100% FP** — 42.199 alert yang semuanya dari DNS resolver internal, seharusnya tidak pernah sampai ke analyst
3. **SYN Flood 99.5% FP** — 37.179 dari 37.359 alert dari traffic internal Azure
4. **Malware detection 100% TP** — 3 alert, semua ancaman nyata
5. **False Positive Rate (FPR) baseline = 99.8%** — target AI: turunkan ke < 20%
