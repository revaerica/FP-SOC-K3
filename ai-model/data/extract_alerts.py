    ...
    try:
        with open(ALERT_FILE, 'r', errors='ignore') as f:
            for line_num, line in enumerate(f, 1):
                ...
                try:
                    # Wazuh format: YYYY-MM-DDTHH:MM:SS.ffffff+0000
                    # Slice to remove milliseconds and timezone for simple, robust parsing
...
def compute_freq_per_minute(alerts):
    ...
    for a in alerts:
        try:
            ...
            if ts_str and len(ts_str) >= 19:
                ts_clean = ts_str[:19]
                a["_ts"] = datetime.strptime(ts_clean, "%Y-%m-%dT%H:%M:%S")
            else:
                a["_ts"] = datetime.min
        except Exception:
            a["_ts"] = datetime.min
